...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authError || !user) {
      console.error('Unauthorized access attempt to bookmark API:', authError);
    ...
    if (!workoutId) {
      console.error('Missing workout ID in request');
      return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (authError || !user) {
      console.error('Unauthorized access attempt to check bookmark:', authError);
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      );
